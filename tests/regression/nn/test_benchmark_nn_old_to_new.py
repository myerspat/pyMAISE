...
def test_benchmark_nn_old_to_new():
    ...
    datasets = [mai.load_MITR(), mai.load_heat(), mai.load_fp(), mai.load_xs()]
    for data_set in datasets:
        ...
        structural_hyperparameters = {
            "dense_input": {
                "units": mai.Int(min_value=200, max_value=300, step=5),
        ...
        model_settings = {
            ...
            "fnn": {
                ...
                "fitting_params": {
                    "batch_size": mai.Int(min_value=8, max_value=32),
                    "epochs": 50,
        ...
        # Setting for new nn model info
        mai.settings.init(settings_changes=settings)
        ...
        assert old_nn_postprocessor.metrics(model_type="nn")["Test R2"].to_numpy()[
            0
        ] == pytest.approx(
            new_nn_postprocessor.metrics(model_type="fnn")["Test R2"].to_numpy()[0],
            plus_minus
            * old_nn_postprocessor.metrics(model_type="nn")["Test R2"].to_numpy()[0],
        )
